...
**测试步骤**：
1. 输入明文为 `HELLO`，设置密钥为 `1100110011001100`。
2. 选择 ASCII 格式，点击“加密”按钮。
3. 系统生成 ASCII 编码格式的密文输出（如下图所示）。
4. 使用相同的密钥进行解密，验证是否还原为 `HELLO`。
...
5. 修改密文中的某一分组，重新解密，观察解密后的明文变化，验证 CBC 模式的链式依赖效果。
...
#### 3. loginSQL.py
- **简介**：数据库管理模块，负责用户信息的存储和管理，支持注册和登录功能。
- **主要功能**：通过 SQL 查询验证用户身份，保护系统的访问权限。
- **重要函数**：
  - `register_user(username, password)`：将新用户信息存储在数据库中。
  - `authenticate_user(username, password)`：验证用户身份信息，确保只有授权用户可以使用加解密服务。

---

#### 4. README.md
- **简介**：项目文档文件，包含关于项目的概述、使用说明和技术细节。
- **内容**：
  - 项目简介：介绍项目的功能和目标。
  - 使用指南：包含项目运行、加解密操作的详细说明。
  - 各模块功能：提供了所有模块的基本信息。
...
#### 6. SBoxBuild.py
- **简介**：生成 S-AES 算法中的 S-Box 表，用于数据的非线性替换。
- **主要功能**：构建 S-Box 表，优化加解密过程中的替换操作。
- **重要函数**：
  - `build_s_box()`：生成 S-Box 替换表，增强加密算法的混淆性。
  - `build_inverse_s_box()`：生成 S-Box 的逆表，用于解密时的逆替换操作。
